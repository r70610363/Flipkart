...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
      ...
      try {
          ...
          if (verifyResponse.success) {
              ...
              if (authMode === 'login') {
                  ...
                  // Note: Login via mobile OTP usually doesn't need password check here if using OTP flow.
                  // But if using Password flow, we check password. 
                  // Since we are using Mobile Auto-OTP for Login, we assume it logs in directly.
                  ...
                  if (currentPhase === 'mobile_otp') {
                      // Mobile verified, DIRECTLY SIGNUP (Skipping Email OTP as requested)
            ...
            if (step === 'input') {
                ...
                if (response.success) {
                    ...
                    setTimeout(() => {
                        ...
                        setOtp(devCode);
                        verifyAndProceed(inputVal, devCode, 'login');
                    }, 5000);
                } else {
            ...
            if (step === 'input') {
                ...
                if (response.success) {
                    ...
                    setTimeout(() => {
                        const devCode = response.devCode || '1234';
                        setOtp(devCode);
                        verifyAndProceed(signupForm.mobile, devCode, 'mobile_otp');
                    }, 5000);
                } else {
                    setError(response.message);
                    setIsLoading(false);
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans relative bg-[#f1f3f6]">
        ...
        <div className="md:hidden px-2 pb-2 bg-[#2874f0]">
           <form onSubmit={handleSearch} className="relative">
              <input
                 ...
                 className="w-full pl-3 pr-10 py-2 rounded-[2px] text-sm focus:outline-none shadow-sm h-[38px] bg-white text-slate-900"
                 value={filters.searchQuery}
                 onChange={(e) => setFilters({ ...filters, searchQuery: e.target.value })}
              />
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-[60] bg-black/50 md:hidden animate-in fade-in duration-200" onClick={() => setIsMenuOpen(false)}>
            ...
            <nav className="flex flex-col text-sm text-slate-700 font-medium">
                ...
                <Link to="/profile" onClick={() => setIsMenuOpen(false)} className="px-5 py-4 border-b border-slate-100 hover:bg-slate-50 flex items-center gap-3"><UserIcon className="w-4 h-4 text-slate-500"/> My Profile</Link>
      ...
      {/* Main Content */}
      <main className="flex-grow w-full mx-auto pb-6">
        {children}
      </main>
                    ...
                    authMode === 'login' ? (
                        ...
                        <div className="relative group">
                            <input
                                ...
                                className="w-full py-2.5 border-b border-slate-300 focus:border-[#2874f0] outline-none transition-colors peer bg-transparent z-10 relative text-slate-800 font-medium"
                                placeholder=" "
                            />
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="relative group">
                                <input
                                    ...
                                    className="w-full py-2.5 border-b border-slate-300 focus:border-[#2874f0] outline-none transition-colors peer bg-transparent z-10 relative text-slate-800 font-medium pr-8"
                                    placeholder=" "
                                />
